...
class Image {
	...
	// 等比缩放
	static function img_resize($srcImage, $rsltname, $type = '', $srcX, $srcY, $myWidth, $myHeight = 0) {
		// 获取原图信息
		$info = Image::getImageInfo ( $srcImage );
		if ($info !== false) {
			$srcWidth = $info ['width'];
			$srcHeight = $info ['height'];
			$type = empty ( $type ) ? $info ['type'] : $type;
			...
			unset ( $info );
			if (empty ( $myWidth )) { // 计算缩放比例
				$scale = $myHeight / $srcHeight;
				$myWidth = ( int ) ($srcWidth * $scale);
			} else if (empty ( $myHeight )) { // 计算缩放比例
				$scale = $myWidth / $srcWidth;
				$myHeight = ( int ) ($srcHeight * $scale);
			} else {
				$scale = max ( $myWidth / $srcWidth, $myHeight / $srcHeight );
			}
			if ($scale >= 1) {
				// 超过原图大小不缩
				$width = $srcWidth;
				...
				// 截取原图尺寸
				$width = ( int ) ($myWidth / $scale);
				$srcX = ( int ) (($srcWidth - $width) / 2);
				$height = ( int ) ($myHeight / $scale);
			}
			// 载入原图
			$createFun = 'ImageCreateFrom' . $type;
			$srcImg = $createFun ( $srcImage );
			// 创建缩略图
			if ($type != 'gif' && function_exists ( 'imagecreatetruecolor' ))
				$rsltImg = imagecreatetruecolor ( $myWidth, $myHeight );
			else
				$rsltImg = imagecreate ( $myWidth, $myHeight );
				//
			if ('gif' == $type || 'png' == $type) {
				$color = imagecolorallocate ( $rsltImg, 255, 255, 255 );
				imagecolortransparent ( $rsltImg, $color );
				imagefill ( $rsltImg, 0, 0, $color );
			}
			// 复制图片
			if (function_exists ( "ImageCopyResampled" )) {
				imagecopyresampled ( $rsltImg, $srcImg, 0, 0, $srcX, $srcY, $myWidth, $myHeight, $width, $height );
			} else {
				imagecopyresized ( $rsltImg, $srcImg, 0, 0, $srcX, $srcY, $myWidth, $myHeight, $width, $height );
			}
			// 生成图片
			$imageFun = 'image' . $type;
			if ($type == 'png') {
				$imageFun ( $rsltImg, $rsltname );
			} else {
				$imageFun ( $rsltImg, $rsltname, 100 );
			}
			imagedestroy ( $rsltImg );
			imagedestroy ( $srcImg );
			return $rsltname;
		}
		return false;
	...
	/*
	 * static function
	 * buildString($string,$rgb=array(),$filename='',$type='png',$disturb=1,$border=true)
	 * { if(is_string($size))		$size	=	explode(',',$size); $width	=	$size[0];
	 * $height	=	$size[1]; if(is_string($font))		$font	=	explode(',',$font);
	 * $fontface	=	$font[0]; $fontsize	 	=	$font[1]; $length		=	strlen($string);
	 * $width = ($length*9+10)>$width?$length*9+10:$width; $height	=	22; if (
	 * $type!='gif' && function_exists('imagecreatetruecolor')) { $im =
	 * @imagecreatetruecolor($width,$height); }else { $im =
	 * @imagecreate($width,$height); } if(empty($rgb)) { $color =
	 * imagecolorallocate($im, 102, 104, 104); }else{ $color =
	 * imagecolorallocate($im, $rgb[0], $rgb[1], $rgb[2]); } $backColor =
	 * imagecolorallocate($im, 255,255,255); //背景色（随机） $borderColor =
	 * imagecolorallocate($im, 100, 100, 100); //边框色 $pointColor =
	 * imagecolorallocate($im,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));
	 * //点颜色 @imagefilledrectangle($im, 0, 0, $width - 1, $height - 1,
	 * $backColor); @imagerectangle($im, 0, 0, $width-1, $height-1,
	 * $borderColor); @imagestring($im, 5, 5, 3, $string, $color);
	 * if(!empty($disturb)) { // 添加干扰 if($disturb=1 || $disturb=3) {
	 * for($i=0;$i<25;$i++){
	 * imagesetpixel($im,mt_rand(0,$width),mt_rand(0,$height),$pointColor); }
	 * }elseif($disturb=2 || $disturb=3){ for($i=0;$i<10;$i++){
	 ...
	 *         +----------------------------------------------------------
	 */
	static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 48, $height = 25, $verifyName = 'verify') {
		import ( '@.ORG.String' );
